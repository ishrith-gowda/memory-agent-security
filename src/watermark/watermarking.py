...
class WatermarkEncoder:
    ...
    def detect(self, content: str, watermark: str) -> float:
        ...
        matches = sum(1 for a, b in zip(watermark[:min_len], extracted[:min_len]) if a == b)
...
def create_watermark_encoder(algorithm: str, config: Optional[Dict[str, Any]] = None) -> WatermarkEncoder:
    """
...
class ProvenanceTracker:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.encoder = create_watermark_encoder(
            self.config.get("algorithm", "composite"),
            config
        )
        self.logger = logger
    
    def register_content(self, content_id: str, content: str, 
